...
export async function POST(req: Request) {
  try {
    ...
    if (last) {
      ...
      if (diff < 30 * 1000) {
        await prisma.anomaly.create({
          data: {
            user_id: student.id, // ⚠️ your schema uses Profile here, but keeping simple
            type: "FAST_SCAN",
            severity: "HIGH",
            description: "Fast repeated scan detected",
          },
        });

        return NextResponse.json(
          { error: "Too fast scan detected" },
          { status: 429 }
        );
      }
    }
    ...
    // CHECK IN / OUT LOGIC
    const isCheckIn = !last || last.check_out !== null;
